...
class RecyclerViewAdapter extends RecyclerView.Adapter<RecyclerViewAdapter.ViewHolder> {
     ...
     */
    public RecyclerViewAdapter(List<ImageView> items, int itemLayout) {
        this.items = items;
    ...
    public void onBindViewHolder(RecyclerViewAdapter.ViewHolder holder, int position) {
        ...
        ImageView item = items.get(position);
        // save information in holder, we have one type in this adapter
        //holder.primaryText.setText(item);
        holder.itemView.setTag(item);
        if ((position % 2) == 0) {
            holder.itemView.setBackgroundResource(R.color.GouiranLightBlue);
        } else {
            holder.itemView.setBackgroundResource(R.color.GouiranLightPink);
        }
    }
    /**
     *
